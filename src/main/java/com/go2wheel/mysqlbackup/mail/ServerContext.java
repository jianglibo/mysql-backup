...
public class ServerContext {
	...
	public MsgKeyAndFills2 healthy() {
		...
		// return new MsgKeyAndFills2(MEMROY_FULL_MESSAGE_KEY, avd,
		// server.getMemoryValve());
		// }
		// }

		// if (getStorageStates() != null) {
		// OptionalDouble od = getStorageStates().stream().mapToDouble(one ->
		// one.getUsedRatio()).max();
		// double avd = od.isPresent() ? od.getAsDouble() : 0d;
		// if (avd > server.getDiskValve()) {
		// return new MsgKeyAndFills2(DISK_FULL_MESSAGE_KEY, avd,
		// server.getDiskValve());
		// }
		// }
	...
	// Map<String, Set<StorageState>> stss =
	// getStorageStates().stream().collect(Collectors.groupingBy(sst -> {
	...
	// Map<String, Set<StorageState>> oneByRoot =
	// dailyData.stream().collect(Collectors.groupingBy(
	// StorageState::getRoot, TreeMap::new, Collectors.mapping(sst -> sst,
	// Collectors.toSet())));
	//
	...
	// {
	// onlyOneRecord.put(oneRootItem.getKey(),
	// oneRootItem.getValue().iterator().next());
	// }
	//
	...
	// Set<StorageState> rootData = es.getValue(); // all storageStates about one
	// root, span multiple days.
	//
	// // date as the key, maybe has multiple values, if we run program twice a day.
	// Map<String, Set<StorageState>> oneByDate =
	// rootData.stream().collect(Collectors.groupingBy(sst ->
	// SQLTimeUtil.formatDate(sst.getCreatedAt()), TreeMap::new,
	// Collectors.mapping(sst -> sst, Collectors.toSet())));
	//
	// // get one record for one root in one day. If there was more than one record
	// discard it.
	// Map<String, StorageState> onlyOneRecord = new TreeMap<>();
	//
	// for(Map.Entry<String, Set<StorageState>> oneDateItem : oneByDate.entrySet())
	// {
	// onlyOneRecord.put(oneDateItem.getKey() /*it's date.*/,
	// oneDateItem.getValue().iterator().next());
	// }
	//
	...
	// Map<String, Set<ServerState>> oneByDate =
	// es.getValue().stream().collect(Collectors.groupingBy(sst -> {
	...
	//
	//
	// for (Map.Entry<String, Map<String, Set<ServerState>>> es :
	// byHours.entrySet()) {
	...
	// double avgMemused =
	// hes.getValue().stream().collect(Collectors.averagingDouble(st -> {
	// double d = (double) st.getMemUsed() / (st.getMemFree() + st.getMemUsed());
	// return d;
	// }));
	// avgMemused = avgMemused * 100;
	// double avgLoad = hes.getValue().stream().collect(Collectors.averagingInt(st
	// -> st.getAverageLoad()));
	...
	public List<PsDiskMemFreeResult> getMemfrees(int num) throws JsonParseException, JsonMappingException, IOException {
		return getServer().getMemoryFreeResult(num);
	}


	public List<PsDiskMemFreeResult> getDiskfrees(int num) throws JsonParseException, JsonMappingException, IOException {
		return getServer().getDiskFreeResult(num);
	}


	public List<PsBorgAchiveResult> getBorgArchiveResults(int num) throws JsonParseException, JsonMappingException, IOException {
		return getServer().getBorgArchiveResult(num);
	}

	public List<PsBorgAchiveResult> getBorgPruneResults(int num) throws JsonParseException, JsonMappingException, IOException {
		return getServer().getBorgPruneResult(num);
	}


	public List<PsMysqldumpResult> getMysqlDumpResults(int num) throws JsonParseException, JsonMappingException, IOException {
		return getServer().getMysqlDumpResult(num);
	}


	public List<PsMysqlflushResult> getMysqlFlushResults(int num) throws JsonParseException, JsonMappingException, IOException {
		return getServer().getMysqlFlushResult(num);
	}
