...
public class PowershellCommandSchedule {
	...
	public void schedule(ConfigFile configFile) throws SchedulerException, ParseException {
		configFile.getCrons().entrySet().stream().forEach(entry -> {
			try {
				createTrigger(configFile,
						entry.getKey(),
						entry.getValue(),
						PowershellExecutionJob.class,
	...
	public boolean createTrigger(ConfigFile configFile, String psCmdKey, String cronExpression, Class<? extends Job> jobClass, JobKey jk, TriggerKey tk) throws SchedulerException, ParseException {
		if (!StringUtil.hasAnyNonBlankWord(cronExpression)) return false;
		JobDetail job = scheduler.getJobDetail(jk);
		if (job == null) {
			job = newJob(jobClass)
					.withIdentity(jk)
					.usingJobData(CommonJobDataKey.JOB_DATA_KEY_ID, configFile.getMypath())
					.usingJobData(CommonJobDataKey.JOB_DATA_PS_COMMAND_KEY, psCmdKey)
					.storeDurably()
					.build();
